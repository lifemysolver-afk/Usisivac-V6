...
## 2025-05-25 - Vectorized Semantic Drift Score Batching in Guardian
**Learning:** Sequential computation of cosine similarity between project essence and multiple agent outputs introduces unnecessary overhead. Batching embedding computation and using matrix-vector multiplication (`action_embs @ essence_emb`) yields >2.3x speedup.
**Action:** Batch embed agent outputs and compute cosine similarity against single target embeddings using matrix-vector dot products.
