...
Na osnovu izvestaja ResearchAgent-a i CriticAgent-a,
samostalno pise Python kod.
Generise: feature engineering, model training, pipeline skripte.
ANTI-SIM: Kod se STVARNO pise na disk, ne simulira.
"""
...
def generate_code(problem: str, research_output: dict, critic_output: dict = None) -> dict:
    """
    Generise Python kod na osnovu research-a i kritike.
    ANTI-SIM: Kod se STVARNO pise na disk.
    """
    ...
    system = (
        ...
        "Pisi cist, produkcioni Python kod. "
        "PRAVILA:\n"
        ...
        "6. Kod mora biti IZVRSIV - ne pseudo-kod\n"
    ...
    prompt = (
        ...
        f"RAG KNOWLEDGE:\n{rag_context}\n\n"
        f"{critic_context}\n\n"
        "Napisi kompletan Python kod koji resava ovaj problem:"
    )

    code = llm_call(prompt, system=system)

    # Ocisti kod od markdown blokova ako ih ima
    if "```python" in code:
        code = code.split("```python")[1].split("```")[0]
    elif "```" in code:
        code = code.split("```")[1].split("```")[0]

    # STVARNO pisi na disk
...
def generate_features(data_description: str, research_output: dict) -> dict:
    """Generise feature engineering skriptu."""
    log_work(AGENT, "FEAT_GEN_START", data_description[:100])

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    system = (
        "Ti si FeatureEngineer. Pisi Python kod za feature engineering. "
        "Koristi pandas, numpy, sklearn. "
        "Svaki feature mora imati komentar zasto je koristan. "
    ...
    prompt = (
        f"DATA:\n{data_description}\n\n"
        f"RESEARCH:\n{research_context}\n\n"
        "Generisi feature engineering pipeline:"
    )
