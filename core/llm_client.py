...
# Thread-safe global requests.Session for HuggingFace connection pooling
_hf_session = None
_hf_session_lock = threading.Lock()

def _get_hf_session() -> requests.Session:
    """Gets or initializes the global thread-safe requests.Session for connection pooling."""
    global _hf_session
...
# Memoized helpers for SDK clients to eliminate initialization overhead on consecutive LLM calls
@functools.lru_cache(maxsize=10)
def _get_groq_client(api_key: str):
    """Retrieves a cached Groq client instance for the given API key."""
...
def _get_openai_client(api_key: str, base_url: Optional[str] = None):
    """Retrieves a cached OpenAI-compatible SDK client instance."""
...
def _get_gemini_client(api_key: str):
    """Retrieves a cached Google GenAI client instance."""
...
def _call_huggingface(prompt: str, model: str = "HuggingFaceH4/zephyr-7b-beta", system: str = "") -> str:
    ...
    response = _get_hf_session().post(API_URL, headers=headers, json=payload)
