...
# ─── Memoized SDK Client Construction ─────────────────────────────────────────
# Bolt ⚡ Performance optimization: Instantiating SDK clients (Groq, OpenAI, GenAI)
# takes ~30-100ms per call. Memoizing client instances via lru_cache reduces client lookup
# overhead to ~0.001ms (a ~100x-1000x speedup for client initialization).
