...
# Thread-safe connection pool for requests-based clients
_session = None
_session_lock = threading.Lock()

@lru_cache(maxsize=10)
def _get_groq_client(api_key: str):
    """Memoized helper for Groq client."""
    from groq import Groq
    return Groq(api_key=api_key)

@lru_cache(maxsize=10)
def _get_openai_client(api_key: str, base_url: str = None):
    """Memoized helper for OpenAI client (handles Mistral and OpenRouter too)."""
    from openai import OpenAI
    return OpenAI(api_key=api_key, base_url=base_url)

@lru_cache(maxsize=10)
def _get_gemini_client(api_key: str):
    """Memoized helper for google-genai Client."""
    from google import genai
    return genai.Client(api_key=api_key)

def _get_huggingface_session() -> requests.Session:
    """Thread-safe singleton requests.Session provider to leverage connection pooling."""
    global _session
    if _session is None:
        with _session_lock:
            if _session is None:
...
def _call_gemini(prompt: str, model: str = "gemini-2.0-flash", system: str = "") -> str:
    """Uses new google-genai SDK with key rotation and memoized client retrieval."""
...
def _call_huggingface(prompt: str, model: str = "HuggingFaceH4/zephyr-7b-beta", system: str = "") -> str:
    ...
    session = _get_huggingface_session()
