...
def _call_groq(prompt: str, model: str = "llama-3.3-70b-versatile", system: str = "") -> str:
    # Use cached client to avoid instantiation overhead and reuse TCP/HTTP sessions
...
def _call_mistral(prompt: str, model: str = "mistral-small-latest", system: str = "") -> str:
    # Use cached OpenAI client for Mistral endpoint to reuse connections
...
def _call_openrouter(prompt: str, model: str = "mistralai/mistral-7b-instruct:free", system: str = "") -> str:
    # Use cached OpenAI client for OpenRouter endpoint
    key = os.getenv("OPENROUTER_API_KEY", "")
    if "\\n" in key:
        key = key.split("\\n")[0]
    base_url = "https://openrouter.ai/api/v1"
    client = _get_openai_client(key, base_url)
    msgs = []
    if system:
        msgs.append({"role": "system", "content": system})
    msgs.append({"role": "user", "content": prompt})
