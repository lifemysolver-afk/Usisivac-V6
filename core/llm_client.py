...
# ─── Memoized Client Instantiation Helpers ─────────────────────────────────────
# Cached SDK client helpers eliminate ~34ms instantiation overhead per Groq call
# and ~1.4ms per OpenAI-compatible call, saving time during loop execution.
...
def _get_hf_session() -> requests.Session:
    # Shared HTTP session provides TCP/TLS connection pooling
