...
# ─── Cached SDK Client & Session Helpers ──────────────────────────────────────
# Performance optimization: Memoize client instantiation per API key / base_url.
# Instantiating Groq, OpenAI, or GenAI clients on every call incurs ~34ms-103ms overhead per call.
# LRU caching reduces client retrieval time to <0.001ms while preserving thread safety.
...
def _get_hf_session():
    # Reuse HTTP connection pool for Hugging Face inference calls (saves ~10-50ms TLS overhead)
...
def _call_mistral(prompt: str, model: str = "mistral-small-latest", system: str = "") -> str:
    ...
    client = _get_openai_client(api_key, "https://api.mistral.ai/v1")
    msgs = []
    if system:
        msgs.append({"role": "system", "content": system})
    msgs.append({"role": "user", "content": prompt})
...
def _call_gemini(prompt: str, model: str = "gemini-2.0-flash", system: str = "") -> str:
    """Uses new google-genai SDK with key rotation and memoized client instances."""
...
def _call_openrouter(prompt: str, model: str = "mistralai/mistral-7b-instruct:free", system: str = "") -> str:
    ...
    if "\\n" in key:
        key = key.split("\\n")[0]
    client = _get_openai_client(key, "https://openrouter.ai/api/v1")
    msgs = []
    if system:
        msgs.append({"role": "system", "content": system})
    msgs.append({"role": "user", "content": prompt})
