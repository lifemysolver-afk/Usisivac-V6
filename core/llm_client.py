...
def _call_groq(prompt: str, model: str = "llama-3.3-70b-versatile", system: str = "") -> str:
    # Bolt: Memoized client instantiation saves ~90ms per call
...
def _call_mistral(prompt: str, model: str = "mistral-small-latest", system: str = "") -> str:
    # Bolt: Memoized client instantiation saves ~80ms per call
    client = _get_openai_client(os.getenv("MISTRAL_API_KEY"), "https://api.mistral.ai/v1")
    msgs = []
    if system:
        msgs.append({"role": "system", "content": system})
    msgs.append({"role": "user", "content": prompt})
...
def _call_gemini(prompt: str, model: str = "gemini-2.0-flash", system: str = "") -> str:
    ...
    # Bolt: Memoized client instantiation saves ~220ms per call
...
# Bolt: Shared session for connection pooling
...
def _call_huggingface(prompt: str, model: str = "HuggingFaceH4/zephyr-7b-beta", system: str = "") -> str:
    ...
    # Bolt: Using shared session for faster subsequent requests (connection pooling)
    response = _hf_session.post(API_URL, headers=headers, json=payload)
