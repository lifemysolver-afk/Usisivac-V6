...
@functools.lru_cache(maxsize=1)
def _get_model():
    """Memoized SentenceTransformer to avoid redundant loads (~18s each)."""
    from sentence_transformers import SentenceTransformer
    return SentenceTransformer(EMBED_MODEL)

class FastSharedEF:
    """Custom Embedding Function that reuses the memoized model."""
...
@functools.lru_cache(maxsize=1)
def _ef():
    """Returns the shared, fast embedding function."""
    return FastSharedEF()
