...
def full_audit(pipeline_results: dict) -> dict:
    ...
    # 1. Drift score za svaki agent output (Vectorized batch embedding for performance)
    drift_scores = {}
    valid_agents = []
    agent_descs = []
    ...
    if valid_agents:
        try:
            ...
            # Embed project essence once and batch embed agent descriptions
            emb_essence = embed(project_essence)
            embs_actions = embed_batch(agent_descs)  # Shape: (N, 384)
            cos_sims = embs_actions @ emb_essence  # Matrix-vector product
            ...
            # Fallback to individual compute_drift_score if vectorization fails
            for agent_name, desc in zip(valid_agents, agent_descs):
