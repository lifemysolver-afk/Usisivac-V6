...
The repository was successfully cloned to `[EXTENSION_ROOT]`. Initial inspection revealed a TypeScript project structure with `package.json` for dependency management and `src` directory containing the core logic.
...
```bash
ubuntu@sandbox:~ $ cd [EXTENSION_ROOT]/extension && npm install
...
ubuntu@sandbox:~ $ cd [EXTENSION_ROOT]/extension && npm run build
...
```
...
```bash
ubuntu@sandbox:~ $ cd [EXTENSION_ROOT]/extension && npm test
...
...
```json
{
  ...
  "session": {
    ...
    "session_dir": "[EXTENSION_ROOT]/.gemini/extensions/pickle-rick/sessions/2026-04-12-c022093c",
    "state_file": "[EXTENSION_ROOT]/.gemini/extensions/pickle-rick/sessions/2026-04-12-c022093c/state.json",
    "state": {
      "active": true,
