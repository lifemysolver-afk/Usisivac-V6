...
```bash
user@sandbox:~ $ gh repo clone kizabgd123/pickle-rick-extension
...
```bash
user@sandbox:~ $ cd ../extension && npm install
...
user@sandbox:~ $ cd ../extension && npm run build
...
```
...
```bash
user@sandbox:~ $ cd ../extension && npm test
...
