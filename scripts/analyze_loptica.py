...
nb_path = Path(Path(__file__).resolve().parent.parent / "data" / "Loptica.ipynb")
...
out_code = Path(Path(__file__).resolve().parent.parent / "data" / "loptica_extracted_code.py")
out_code.write_text(full_code)
print(f"\n=== SAVED: {out_code} ({len(full_code)} chars) ===")

# Sačuvaj markdown
out_md = Path(Path(__file__).resolve().parent.parent / "data" / "loptica_extracted_md.md")
out_md.write_text("\n\n---\n\n".join(all_md))
...
out_analysis = Path(Path(__file__).resolve().parent.parent / "data" / "loptica_analysis.json")
