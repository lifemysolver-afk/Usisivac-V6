...
nb_path = Path("/app/data/Loptica.ipynb")
...
out_code = Path("/app/data/loptica_extracted_code.py")
out_code.write_text(full_code)
print(f"\n=== SAVED: {out_code} ({len(full_code)} chars) ===")

# Sačuvaj markdown
out_md = Path("/app/data/loptica_extracted_md.md")
out_md.write_text("\n\n---\n\n".join(all_md))
...
out_analysis = Path("/app/data/loptica_analysis.json")
