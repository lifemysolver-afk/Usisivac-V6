...
def test_exactly_quorum_of_3_passes():
    ...
    def _get_vote_fn(persona, persona_prompt, action, context):
        # First 3 personas pass, last 2 veto (but LEGAL must PASS so we don't trigger the prioritised LLM LEGAL veto check)
        # Ensure 'LEGAL' is in the passing set or at least does not return VETO
        if persona == "LEGAL":
            return "PASS", "ok"
        # Since LEGAL passes, let's make 2 other personas pass and the remaining 2 veto
        other_personas = [p for p in personas if p != "LEGAL"]
        if persona in other_personas[:2]:
            return "PASS", "ok"
        return "VETO", "no"
